    ...
    <>
      ...
      >
        <ReactMarkdown
          ...
          components={{
            // Rendered as a real anchor so it looks, focuses and reads like a link; the
            // container's handler is what stops it navigating. `rel` and `target` are set here
            // rather than left to the interstitial so that a reader who opens one in a new tab
            // from the context menu gets them too.
            a: ({ node: _node, ...props }) => (
      ...
      {pending !== null && (
        <Warning role="alert">
          ...
          <WarningActions>
            ...
            <Control type="button" onClick={() => setPending(null)}>
              Cancel
            </Control>
          </WarningActions>
        </Warning>
