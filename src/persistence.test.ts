...
describe("shouldPersistQuery", () => {
  ...
  it("refuses a query nobody has thought about yet", () => {
    // Tickets 06, 08 and 10 each add a read. None of them is persisted by default, and the
    // question to answer before adding one is whether its value is plain JSON.
    expect(shouldPersistQuery(["courtParameters", "34"])).toBe(false);
