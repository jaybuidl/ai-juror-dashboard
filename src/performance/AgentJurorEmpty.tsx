...
export function AgentJurorEmpty({
  ...
  /**
   * The disputes this claim may actually be made over: read, and with a panel in them.
   *
   * Ticket 17 held the undrawn rows out of `Sparsity.emptyColumns` because a dispute the court
   * has not drawn a panel for has no draw in *any* column — so counting it would report an agent
    ...
    <>
      <Empty aria-labelledby="never-drawn-heading">
        <Said>
          ...
          <EmptyBody>
            Kleros draws jurors at random, weighted by stake. Across the {drawnDisputes}{" "}
            {drawnDisputes === 1 ? "dispute" : "disputes"} the court has drawn a panel for and the{" "}
            ...
            {/* The second kind of blank, which ticket 17 separated from sparsity on the matrix and
                which this sentence would otherwise absorb into it. A dispute with no panel yet has
                no draw in any column, so it is evidence about the court's schedule and none at all
                about this agent juror. Named by id for the reason the sparsity note names them:
                the reader can then see which rows the claim above is not about. */}
